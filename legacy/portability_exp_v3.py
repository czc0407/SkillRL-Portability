...
class Agent:
    ...
    def get_action_from_model(self, obs: str) -> str:
        ...
        for step in range(args.max_steps):
            ...
            for i in range(args.env_num):
                ...
                if skill_bank is not None:
                    ...
                    if s:
                        obs_text = s + "\n\n" + obs_text
                actions.append(agent.get_action_from_model(obs_text))
...
if __name__ == "__main__":
    os.makedirs("logs/portability", exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(message)s",
        handlers=[
            logging.FileHandler(
                f"logs/portability/{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
                encoding="utf-8"),
    ...
    p.add_argument("--skills_json",
                   default="memory_data/alfworld/claude_style_skills.json")
    ...
    p.add_argument("--output_dir",     default="results/portability")
